...
async def geocode(db: Session, query: str) -> dict | None:
    ...
    sicily = [
        x
        for x in raw
        if "sicilia" in x.get("display_name", "").casefold()
    ]
    ...
    result = (
        ...
        if not chosen
            ...
            "osmType": chosen.get("osm_type"),
            "osmId": chosen.get("osm_id"),
        }
    )
...
async def osrm_route(
    ...
    try:
        ...
        value = {
            ...
            "updatedAt": datetime.now(ROME).isoformat(),
            "distanceKm": round(item["distance"] / 1000, 1),
            "durationMinutes": round(item["duration"] / 60),
...
async def tomtom_route(
    ...
    url = (
        ...
        f"{destination['lat']},{destination['lon']}/json"
    )
    ...
    try:
        ...
        value = {
            ...
            "updatedAt": datetime.now(ROME).isoformat(),
            "liveDurationMinutes": live,
            "baseDurationMinutes": no_traffic,
            "trafficDelayMinutes": max(0, live - no_traffic),
            "distanceKm": round(
...
def traffic_delay_minutes(
    ...
    static_seconds: float | None,
) -> int | None:
    if live_seconds is None or static_seconds is None:
        return None

    return round(
        max(0, live_seconds - static_seconds) / 60
    )
...
async def google_route(
    ...
    if not settings.google_routes_api_key:
        return {
            ...
            "dataState": "NOT_CONFIGURED",
            "message": "GOOGLE_ROUTES_API_KEY non configurata",
        }
    ...
    if travel_mode is None:
        return {
            "provider": "Google Routes API",
            "dataState": "ERROR",
            "message": f"Modalità non supportata: {mode}",
        }

    preference = (
        "TRAFFIC_AWARE"
        if travel_mode == "DRIVE" and traffic
    ...
    key = (
        f"route:google:{travel_mode}:"
        f"{preference or 'DEFAULT'}:"
        f"{origin['lat']:.5f},{origin['lon']:.5f}:"
        f"{destination['lat']:.5f},{destination['lon']:.5f}"
    )
    ...
    if preference:
        payload["routingPreference"] = preference
    ...
    try:
        ...
        live_seconds = duration_seconds(
            item.get("duration")
        )

        static_seconds = duration_seconds(
            item.get("staticDuration")
        )

        if travel_mode == "DRIVE":
            duration_seconds_value = (
                live_seconds or static_seconds
            )
        else:
            duration_seconds_value = live_seconds

        encoded = (
            item.get("polyline") or {}
        ).get("encodedPolyline")
        ...
        value = {
            ...
            "staticDurationMinutes": (
                round(static_seconds / 60)
                if static_seconds is not None
                else None
            ),
            "liveDurationMinutes": (
                round(live_seconds / 60)
                if (
                    live_seconds is not None
                    and travel_mode == "DRIVE"
                    and traffic
                )
                else None
            ),
            ...
            "durationMinutes": (
                round(duration_seconds_value / 60)
            ...
            "geometry": (
                {
                    "type": "LineString",
                    "coordinates": decode_google_polyline(
                        encoded
                    ),
                }
                if encoded
...
def parse_ingv_latest(
    ...
    match = re.search(
        (
            r"(COMUNICATO DI ATTIVITA.? VULCANICA\s*"
            r"-?\s*ETNA.*?)(?="
            r"ARCHIVIO AGGIORNAMENTI|VULCANICI)"
        ),
        text,
        re.I,
    )
    ...
    return {
        ...
        "summary": body[:900].strip(),
        "timestamp": (
            timestamp.group(1) + " UTC"
            if timestamp
            else None
        ),
    }
...
async def etna_latest(
    db: Session,
) -> dict:
    key = "etna:ingv:latest"
    cached, fresh = cache_get(db, key)
    ...
    url = (
        ...
        "sezioniesterne/UltimiAggiornamenti.php"
    )
    ...
    try:
        ...
        parsed = parse_ingv_latest(
            response.text
        )

        if not parsed:
            raise ValueError(
                "Formato INGV non riconosciuto"
            )

        value = {
            **parsed,
            "provider": "INGV Osservatorio Etneo",
            "sourceUrl": url,
            "updatedAt": datetime.now(ROME).isoformat(),
            "dataState": "LIVE",
        }
